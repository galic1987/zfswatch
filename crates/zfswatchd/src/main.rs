    ...
    match request {
        ...
        DaemonRequest::Status => {
            ...
            DaemonResponse::Status {
                ...
                devices_connected: Vec::new(), // TODO: track devices
        ...
        DaemonRequest::ListPools => {
            let state = state.lock().await;
            match state.pool_manager.list_pools().await {
        ...
        DaemonRequest::ImportPool {
            ...
            match state
                .pool_manager
                .import_pool(&pool_name, device_path.as_ref())
                .await
            {
        ...
        DaemonRequest::ExportPool { pool_name, force } => {
            let state = state.lock().await;
            match state.pool_manager.export_pool(&pool_name, force).await {
                Ok(()) => DaemonResponse::Success {
        ...
        DaemonRequest::Mount {
            ...
            let result: zfswatch_core::Result<std::path::PathBuf> = if let Some(pass) = passphrase {
                match EncryptionManager::new().load_key(&dataset, &pass).await {
                    Ok(()) => DatasetManager::new().mount(&dataset).await,
                    Err(e) => Err(e),
                }
            } else {
                DatasetManager::new().mount(&dataset).await
            };

            match result {
                Ok(mountpoint) => DaemonResponse::Success {
                    message: format!("Dataset '{dataset}' mounted at {mountpoint:?}"),
                },
                Err(e) => DaemonResponse::Error {
        ...
        DaemonRequest::Unmount {
            dataset,
            unload_key,
        } => {
            let state = state.lock().await;
            match state.dataset_manager.unmount(&dataset, unload_key).await {
                Ok(()) => DaemonResponse::Success {
        ...
        DaemonRequest::InitPool {
            ...
            match PoolManager::new()
                .create_pool(&pool_name, &device_path, &passphrase, &options)
                .await
            {
                Ok(()) => DaemonResponse::Success {
                    message: format!(
                        "Pool '{pool_name}' created on {device_path:?} with encryption"
                    ),
                },
                Err(e) => DaemonResponse::Error {
        ...
        DaemonRequest::ChangeKey {
            dataset,
            old_passphrase,
            new_passphrase,
        } => {
            match EncryptionManager::new()
                .change_key(&dataset, &old_passphrase, &new_passphrase)
                .await
            {
                Ok(()) => DaemonResponse::Success {
